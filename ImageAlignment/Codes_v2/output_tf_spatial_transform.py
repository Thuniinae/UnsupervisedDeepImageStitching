...
def Stitching_Domain_STN(inputs, H_tf, size, resized_shift):
    ...
    def _meshgrid(width_max, width_min, height_max, height_min):
        ...
        x_t = tf.matmul(tf.ones(shape=tf.stack([tf.cast(height, tf.int32), 1])),
                        tf.transpose(a=tf.expand_dims(tf.linspace(width_min,  width_max, tf.cast(width, tf.int32)), 1), perm=[1, 0]))
    ...
    def _transform(image_tf, H_tf, width_max, width_min, height_max, height_min):
        ...
        num_batch = tf.shape(input=image_tf)[0]
        num_height = tf.shape(input=image_tf)[1]
        num_width = tf.shape(input=image_tf)[2]
        num_channels = tf.shape(input=image_tf)[3]
        #  Changed
    ...
    width_max = tf.reduce_max(input_tensor=width_list_tf)
    width_min = tf.reduce_min(input_tensor=width_list_tf)
    height_max = tf.reduce_max(input_tensor=height_list_tf)
    height_min = tf.reduce_min(input_tensor=height_list_tf)
    print("height_min")
