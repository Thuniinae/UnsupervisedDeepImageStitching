...
def transform(image2_tensor, H_tf):
    ...
    def _transform( image2_tensor, H_tf):
        #with tf.variable_scope('_transform'):
            num_batch = tf.shape(input=image2_tensor)[0]
            height = tf.shape(input=image2_tensor)[1]
            width = tf.shape(input=image2_tensor)[2]
            num_channels = tf.shape(input=image2_tensor)[3]
            #  Changed
